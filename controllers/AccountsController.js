...
router.route('/remove_account').post(function (req, res) {
    var sql = "DELETE FROM Accounts WHERE account_id = " + mysql.escape(req.query.account_id) + "LIMIT 1";
...
router.route('/count').get(function (req, res) {
    ...
    for (var i=0, len=tables.length; i<len; i++) {
        sql += "(SELECT COUNT(*) FROM " + tables[i] + " WHERE account_id = " + mysql.escape(req.query.account_id) + ") AS " + colNames[i] + ", ";
    }
    // Remove last comma
...
router.route('/clean_account').post(function (req, res) {
    ...
    var sql = "DELETE FROM " + tables.join(" ") + " WHERE account_id = " + mysql.escape(req.query.account_id);
...
router.route('/settings').get(function (req, res) {
    ...
    var sql = "SELECT " + fields.join(", ") + " FROM Accounts WHERE account_id = " + mysql.escape(req.query.account_id) + "LIMIT 1";
